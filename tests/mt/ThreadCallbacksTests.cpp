	...
	namespace tests
	{
		begin_test_suite( ThreadCallbacksTests )
			...
			test( ThreadExitsAreCalledInTheCorrespondingThread )
			{
				...
				// ACT
				thread t1([&] {
					ntids[0] = tasker::tests::this_thread::get_native_id();
					callbacks->at_thread_exit([&] {
						ntids_atexit[0] = tasker::tests::this_thread::get_native_id();
					});
					callbacks->at_thread_exit([&] {
						ntids_atexit[3] = tasker::tests::this_thread::get_native_id();
					});
				});
				thread t2([&] {
					ntids[1] = tasker::tests::this_thread::get_native_id();
					callbacks->at_thread_exit([&] {
						ntids_atexit[1] = tasker::tests::this_thread::get_native_id();
					});
				});
				thread t3([&] {
					ntids[2] = tasker::tests::this_thread::get_native_id();
					callbacks->at_thread_exit([&] {
						ntids_atexit[2] = tasker::tests::this_thread::get_native_id();
					});
				});
			...
			test( DestructorObjectsAreReleasedAfterExecuted )
			{
				...
				// ASSERT
				assert_equal(1, alive.use_count());
			}
		end_test_suite
	}
}
